...
export function AutoAssignDialog({ planId, scenarioId, guests, tables, assignments, constraints, onClose }: Props) {
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader><DialogTitle className="flex items-center gap-2"><Sparkles size={18} className="text-primary"/>Auto-assign seating</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">Groups guests by party, honors must / must-not constraints, and fills tables to capacity.</p>
          <label className="flex items-center gap-2 text-sm"><Checkbox checked={includeMaybe} onCheckedChange={v => setIncludeMaybe(!!v)}/>Include &ldquo;maybe&rdquo; RSVPs</label>
          <label className="flex items-center gap-2 text-sm"><Checkbox checked={keepExisting} onCheckedChange={v => setKeepExisting(!!v)}/>Keep existing assignments (only fill empty seats)</label>
          <Button variant="outline" onClick={compute} className="w-full">Compute preview</Button>
          {preview && (
            <div className="rounded-lg border border-border/60 p-3 text-sm">
              <div className="font-medium">Preview</div>
              <div className="text-muted-foreground">Seats {seatedNow} of {eligible} eligible guests across {tables.length} tables.</div>
            </div>
          )}
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
          <Button disabled={!preview || loading} onClick={apply}>Apply</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
